...
void uart_printf(UART_HandleTypeDef *huart,const char *format, ...)
{
    ...
    HAL_UART_Transmit_DMA(huart,UserTxBufferFS, length);   //ֻ��Ҫ�����������һֱ������ƽ̨
...
/* USER CODE BEGIN 1 */
#include "filter.h"
uint8_t uart3_rx_buff[UART3_RXBUFFERSIZE];//����3���ջ�����
uint8_t uart6_rx_buff[UART6_RXBUFFERSIZE];//����6���ջ�����
uint8_t uart4_rx_buff[UART4_RXBUFFERSIZE];//����4���ջ�����
uint8_t uart1_rx_buff[UART1_RXBUFFERSIZE];//����1���ջ�����
					...
					switch(uart3_rx_buff[i+1])
					{
						...
						case 0x02:
							...
							if(my_aircraft.status == 0x01)//����������
							{
								...
								pid_enable(&AttitudeControl.internal_pid.x,1);//�����ɻ�ʹ��pid
								pid_enable(&AttitudeControl.internal_pid.y,1);//�����ɻ�ʹ��pid
								pid_enable(&AttitudeControl.internal_pid.z,1);//�����ɻ�ʹ��pid
								pid_enable(&AttitudeControl.external_pid.x,1);//�����ɻ�ʹ��pid
								pid_enable(&AttitudeControl.external_pid.y,1);//�����ɻ�ʹ��pid
								pid_enable(&AttitudeControl.external_pid.z,1);//�����ɻ�ʹ��pid
								AttitudeControl.yaw_target_angle = *AttitudeControl.sensor.yaw;
							}
							else if(my_aircraft.status == 0x00)//�رշ�����
							{
								pid_enable(&AttitudeControl.internal_pid.x,0);//�رշɻ���ʹ��pid
								pid_enable(&AttitudeControl.internal_pid.y,0);//�رշɻ���ʹ��pid
								pid_enable(&AttitudeControl.internal_pid.z,0);//�رշɻ���ʹ��pid
								pid_enable(&AttitudeControl.external_pid.x,0);//�رշɻ���ʹ��pid
								pid_enable(&AttitudeControl.external_pid.y,0);//�رշɻ���ʹ��pid
						...
						case 0x04:
							...
							{
								case 0x01:
									last_tick = now_tick;//������һ��ʱ��
									now_tick = HAL_GetTick();//��ȡ��ǰʱ��
								...
								case 0x02:
									...
									{	
										PositionControl.auto_pos_control_isEnable = 1;//�����Զ�����
...
void TOF_get_data(uint8_t *uart_data,uint16_t size)
{
  ...
  static Filter_LPF_1 TOF_LPF_1 = {0,0,0.0001};
    ...
    TOF.is_valid = 1;//TOF 有效
      ...
      /***��ͨ�˲�end***/
			TOF.distance_mm = (TOF.distance_mm*cosf(my_ahrs.Angle_Data.pitch*DegtoRad)*cosf(my_ahrs.Angle_Data.roll*DegtoRad)+TOF.d_center_x_err_mm*sinf(my_ahrs.Angle_Data.pitch*DegtoRad))+TOF.distance_offset_mm;//TOF�߶Ƚ���
...
void OpticalFlow_get_data(uint8_t *uart_data,uint16_t size)
{
	...
	if(uart_data[0] == 0xFE && uart_data[1] == 0x0A && uart_data[size - 1] == 0x55)
	{
		...
		OpticalFlow.now_yaw = my_ahrs.Angle_Data.yaw;
		OpticalFlow.last_pitch = OpticalFlow.now_pitch;
		OpticalFlow.now_pitch = my_ahrs.Angle_Data.pitch;
		OpticalFlow.last_roll = OpticalFlow.now_roll;
		OpticalFlow.now_roll = my_ahrs.Angle_Data.roll;	
		...
		if(OpticalFlow.valid == 0x00)
		{
				return;
		}
		else if(OpticalFlow.valid == 0xF5)
		{
			OpticalFlow.flow_yaw_x_com_mm = OpticalFlow.d_center_x_err_mm*(1-cosf(yaw_err*DegtoRad));//X�Ჹ��
			OpticalFlow.flow_yaw_y_com_mm = OpticalFlow.d_center_x_err_mm*sinf(yaw_err*DegtoRad);//Y�Ჹ��
			OpticalFlow.flow_pitch_x_com_mm =TOF.distance_mm*tanf(pitch_err*DegtoRad);//pitch��ת����		
			OpticalFlow.flow_roll_y_com_mm = TOF.distance_mm*tanf(roll_err*DegtoRad);//roll��ת����

			
			OpticalFlow.flow_y_integral_mm = OpticalFlow.flow_y_integral*TOF.distance_mm*0.0001f;//����10000���Ը߶�(mm)��Ϊʵ��λ��(mm)
			OpticalFlow.flow_x_integral_mm = OpticalFlow.flow_x_integral*TOF.distance_mm*0.0001f;//����10000���Ը߶�(mm)��Ϊʵ��λ��(mm)
			
			OpticalFlow.flow_y_integral_mm -=OpticalFlow.flow_yaw_y_com_mm  ;//���ػ���yaw��ת����
			OpticalFlow.flow_x_integral_mm -=OpticalFlow.flow_yaw_x_com_mm  ;//���ػ���yaw��ת����

			OpticalFlow.flow_y_integral_mm -=OpticalFlow.flow_roll_y_com_mm  ;//���ػ���roll��ת����
			OpticalFlow.flow_x_integral_mm +=OpticalFlow.flow_pitch_x_com_mm  ;//���ػ���pitch��ת����
			
			OpticalFlow.flow_x_speed = (float)(OpticalFlow.flow_x_integral_mm*1000.0f)/(float)(OpticalFlow.integration_timespan);//�����ù���x���ٶ� ��λm/s
			OpticalFlow.flow_y_speed = (float)(OpticalFlow.flow_y_integral_mm*1000.0f)/(float)OpticalFlow.integration_timespan;//�����ù���y���ٶ� ��λm/s
			if(fabs(OpticalFlow.flow_x_integral_mm) > 0.01f)
			{
				OpticalFlow.flow_x_pos += OpticalFlow.flow_x_integral_mm*0.001f;//���ֵõ�Xλ��
			}
			if(fabs(OpticalFlow.flow_y_integral_mm) >0.02f)
			{
					OpticalFlow.flow_y_pos += OpticalFlow.flow_y_integral_mm*0.001f;//���ֵõ�Yλ��
			}			
		}
	}
...
void aircraft_data_update()
{
	...
	my_aircraft.ROLL = (int8_t)my_ahrs.Angle_Data.roll;
	my_aircraft.PITCH = (int8_t)my_ahrs.Angle_Data.pitch;
	my_aircraft.YAW = (int8_t)my_ahrs.Angle_Data.yaw;
	
	my_aircraft.X_speed = (int16_t)(OpticalFlow.flow_x_speed*100.0f);
	my_aircraft.Y_speed = (int16_t)(OpticalFlow.flow_y_speed*100.0f);
	
	if(rx_data_tim_cnt>100)
	{
		my_aircraft.signal = 100*((float)rx_data_cnt/(float)rx_data_tim_cnt);
		my_aircraft.error_rate = (100-100*((float)rx_data_correct_cnt/(float)rx_data_cnt));
		rx_data_cnt=0;
		rx_data_correct_cnt=0;
		rx_data_tim_cnt=0;
...
void aircraft_data_send()
{
	...
	remote_data_buf[0] = 0x5A;//֡ͷ
	remote_data_buf[1] = 0x01;//��ϢID
	remote_data_buf[2] = my_aircraft.Throttle;//����
	remote_data_buf[3] = my_aircraft.Height>>8;//�߶�
	remote_data_buf[4] = my_aircraft.Height&0xff;//�߶�
	remote_data_buf[5] = my_aircraft.Motor_PWM_duty[0];//���1
	remote_data_buf[6] = my_aircraft.Motor_PWM_duty[1];//���2
	remote_data_buf[7] = my_aircraft.Motor_PWM_duty[2];//���3
	remote_data_buf[8] = my_aircraft.Motor_PWM_duty[3];//���4
	remote_data_buf[9] = my_aircraft.ROLL;//�����
	remote_data_buf[10] = my_aircraft.PITCH;//������
	remote_data_buf[11] = my_aircraft.YAW;//ƫ����
	remote_data_buf[12] = my_aircraft.Battery_Volt;//��ص�ѹ
	remote_data_buf[13] = (uint16_t)my_aircraft.Altitude>>8;//��ѹ�ƺ���
	remote_data_buf[14] = (uint8_t)my_aircraft.Altitude;
	remote_data_buf[15] = (int8_t)my_aircraft.Temperature;//��ѹ���¶�
	remote_data_buf[16] = my_aircraft.status;//��������ǰ״̬
	remote_data_buf[17] = my_aircraft.X_speed>>8;//������Xˮƽ�ٶ�
	remote_data_buf[18] = (uint8_t)my_aircraft.X_speed;
	remote_data_buf[19] = my_aircraft.Y_speed>>8;//������Yˮƽ�ٶ�
	remote_data_buf[20] = (uint8_t)my_aircraft.Y_speed;
	remote_data_buf[21] = my_aircraft.signal;//�����������ź� 
	remote_data_buf[22] = my_aircraft.error_rate;//�������ź�������
	remote_data_buf[AIRCFAFT_DATA_NUM-1] = 0xA5;//֡β
	if(HAL_DMA_GetState(&hdma_usart3_tx) == HAL_DMA_STATE_READY)
		HAL_UART_Transmit_DMA(&huart3,remote_data_buf,AIRCFAFT_DATA_NUM);//�ϴ�������ʵʱ������ң����
