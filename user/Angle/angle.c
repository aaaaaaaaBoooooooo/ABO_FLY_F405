...
void IMU_Calibration(void)
{
	...
	do
	{
		...
		for(i=0; i<Gyro_NUM; i++)//采集陀螺仪数据计算均值
		{
			...
			/***单位转换为°/s***/
			gyro[i].x = (icm42688_data.gyro_x)*Gyro_Gain;
			gyro[i].y = (icm42688_data.gyro_y)*Gyro_Gain;
			gyro[i].z = (icm42688_data.gyro_z)*Gyro_Gain;
		...
		for(i=0;i<Acc_NUM;i++)//采集加速度数据计算均值
		{
			...
			/***单位转换为 g(m/s^2)***/
			acc[i].x = (icm42688_data.acc_x)*Acc_Gain;
			acc[i].y = (icm42688_data.acc_y)*Acc_Gain;
			acc[i].z = (icm42688_data.acc_z)*Acc_Gain;
		...
		if( gyro_var.x<VAR_GyX && gyro_var.y<VAR_GyY && gyro_var.z<VAR_GyZ&&acc_var.x<VAR_AcX && acc_var.y<VAR_AcY &&acc_var.z<VAR_AcZ)//方差足够小
		{
			...
			my_ahrs.IMU_Data.acc_offset.x = 0.0f;
			my_ahrs.IMU_Data.acc_offset.y = 0.0f;
			my_ahrs.IMU_Data.acc_offset.z = 0.0f;
			my_ahrs.IMU_Data.acc_Scale_K.x = 1.0f;
			my_ahrs.IMU_Data.acc_Scale_K.y = 1.0f;
			my_ahrs.IMU_Data.acc_Scale_K.z = 1.0f;	
#endif
			return;
		}
...
{
	/***IMU原始数据更新***/
	icm42688_get_gyro_acc();

	/***IMU角速度数据更新+去零偏   单位为°/s***/
	my_ahrs.IMU_Data.gyro.x= (icm42688_data.gyro_x)*Gyro_Gain-my_ahrs.IMU_Data.gyro_offset.x;
	my_ahrs.IMU_Data.gyro.y= (icm42688_data.gyro_y)*Gyro_Gain-my_ahrs.IMU_Data.gyro_offset.y;
...
#ifdef SIX_CAL
	my_ahrs.IMU_Data.acc.x=(icm42688_data.acc_x*Acc_Gain - my_ahrs.IMU_Data.acc_offset.x)*my_ahrs.IMU_Data.acc_Scale_K.x;//IMU加速度数据更新 六面校准(影响很小)      // 单位为 g(m/s^2)
	my_ahrs.IMU_Data.acc.y=(icm42688_data.acc_y*Acc_Gain - my_ahrs.IMU_Data.acc_offset.y)*my_ahrs.IMU_Data.acc_Scale_K.y ;
	my_ahrs.IMU_Data.acc.z=(icm42688_data.acc_z*Acc_Gain - my_ahrs.IMU_Data.acc_offset.z)*my_ahrs.IMU_Data.acc_Scale_K.z ;
#else
	my_ahrs.IMU_Data.acc.x=(icm42688_data.acc_x)*Acc_Gain;
	my_ahrs.IMU_Data.acc.y=(icm42688_data.acc_y)*Acc_Gain;
	my_ahrs.IMU_Data.acc.z=(icm42688_data.acc_z)*Acc_Gain;
#endif
...
{		
	volatile struct V{
				...
				float y;
				float z;
				} halfGravity,//角速度通过四元数旋转矩阵获得的理论重力加速度向量的一半
	...
	float INV_TAU = 0.15f;//Mahony滤波PI控制器时间常数的倒数，调整来控制对加速度计的相信程度，越大越相信加速度计
...
/*******************************************************************************
** 函数名称: mmu_angle_update(void) 
** 功能描述: 基于磁力计地磁数据计算角度信息
** 参数说明:  
** 返回说明: None
** 创建人员: aBo
** 创建日期: 2025-4-29
...
void mmu_angle_update(void)
{
	...
	if(fabs(my_ahrs.Angle_Data.roll)<10.0f&&fabs(my_ahrs.Angle_Data.pitch)<10.0f)//水平状态下融合
	{
		...
		{
			my_ahrs.Angle_Data.yaw += (my_MMU.angle.yaw-my_ahrs.Angle_Data.yaw)*0.1f;//一阶互补滤波融合磁力计航向角和陀螺仪积分值
		}
	}
	else//倾斜状态下融合
	{
		...
		{
			my_ahrs.Angle_Data.yaw += (my_MMU.angle.yaw-my_ahrs.Angle_Data.yaw)*0.01f;//一阶互补滤波融合磁力计航向角和陀螺仪积分值
		}
